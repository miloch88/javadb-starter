...
public class ConnectionViaDriverManager {
    ...
    /**
     * Url do bazy danych musi być podany w postaci: <br />
     *  {prefix}//{adres_serwera}:{port}/{nazwa_bazy_danych}?{parametry}<br />
     * np.: jdbc:mysql://localhost:3306/jdbc_test?useSSL=false&serverTimezone=Europe/Warsaw<br />
     *
     * {prefix} - identyfikuje rodzaj bazy danych z którą chcemy się połączyć, dzięki niemu można znaleźć odpowiedni sterownik do bazy <br />
     * {adres_serwera} - oznacza domenę lub adres IP serwera na którym znajduje się serwer bazy danych<br />
     * {port} - port pod którym znajduje się baza danych, dla MySQL domyślnym portem jest: 3306<br />
     * {nazwa_bazy_danych} - nazwa bazy z którą chcemy się połączyć<br />
     * {parametry} - dodatkowe parametry, które chcemy przekazać przy łączeniu się z bazą - są opcjonalne<br />
     */
